...
通过 `dws` 命令管理钉钉产品能力。所有命令由 DSH 的 bash 工具在终端中执行，agent 负责生成正确的 CLI 命令。
...
3. **环境变量**：dsh-dingtalk 插件运行时会自动注入以下环境变量，dws CLI 会自动读取，无需手动设置：
   - `DWS_CHANNEL=dsh-dingtalk` — 标识调用来源为 dsh-dingtalk 插件
